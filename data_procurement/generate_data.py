...
col_names = ['RH', 'N_H+(mol)', 'N_NH4+(mol)','N_Na+(mol)', 'N_SO42-(mol)',
             ...
             'P_NH3(atm)', 'P_H2SO4(atm)', 'P_HBr(atm)', 'contains_solid']
...
#list ot hold whether a column contains solid(1 if yes, 0 if no)
contains_solid = []
...
for filename in os.listdir('CSV\'s'):
    ...
    for line_bytes in r.iter_lines():
        ...
        if water_content_lines:
            ...
            if len(wordlist) <= count + 8:
                contains_solid.append(0)
            else:
                contains_solid.append(1)
...
data = pd.DataFrame(zip(relative_humidities, mol_values['H+'], 
                            ...
                            pressure_vals['HBr'], contains_solid),
                            columns=col_names)
'''
